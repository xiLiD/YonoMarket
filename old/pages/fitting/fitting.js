...
Page({
  ...
  deletes: function() {
    ...
    wx.showModal({
      ...
      success: function(res) {
        console.log(res, '撒大撒')
        if (res.confirm) {
          console.log('请求删除购物车接口')
          wx.request({
            url: app.globalData.requestHost + app.globalData.Delete_Cart, //删除购物车
            data: {
              token: wx.getStorageSync('token'),
  ...
  btn_submit_order: function() {
    ...
    wx.showModal({
      ...
      content: '确认购买吗',
      success: function(res) {
        console.log(res, '是否购买？')
        if (res.confirm) {
          console.log('请求结算 回显 接口')
          wx.request({
            url: app.globalData.requestHost + app.globalData.Set_Tle, //结算 回显 接口
            data: {
              token: wx.getStorageSync('token'),
            ...
            success: function(res) {
              ...
              wx.navigateTo({
                url: '/pages/qrdd/qrdd?rels=' + JSON.stringify(res.data.data) + '&cartJSONStr=' + JSON.stringify(idarr)
              })
            },
          })
        }
  ...
  getGouwc: function(page) {
    ...
    that.setData({ [list]: 1})
    wx.request({
      url: app.globalData.requestHost + app.globalData.Cart_List,
      data: data,
      header: {
        "Content-Type": "application/x-www-form-urlencoded"
      },
      ...
      success: function(res) {
        if(res.data.data){
          ...
          //     that.setData({ [hasData]: false, [type]: 2 })
          //   }
          // },1000)
