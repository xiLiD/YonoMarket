...
Component({
  ...
  methods: {
    _onInit() {
      ...
      mdate.dateString = mdate.year + '-' + mdate.month + '-' + mdate.day + ' ' + mdate.time + ':' + '00';
      
      pickerArray.splice(0, 1);
    ...
    pickerChange: function (e) {
      ...
      const month = this.data.pickerArray[0][indexArr[0]].id;
      const day = this.data.pickerArray[1][indexArr[1]].id;
      const time = this.data.pickerArray[2][indexArr[2]].id;
      ...
      date.dateString = date.year + '-' + date.month + '-' + date.day + ' ' + date.time + ':' + '00';
      // console.log(date);
      this.setData({
    ...
    pickerColumnChange: function (e) {
      ...
      if (e.detail.column == 0) {
        let dayNum = this._getNumOfDays(date.getFullYear(), e.detail.value + 1);
        let day = [];
        for (let i = 1; i <= dayNum; i++) {
          day.push({ id: i, name: i + "日" });
        }
        if (dayNum < data.pickerIndex[2] + 1) {
          data.pickerIndex[1] = dayNum - 1;
        }
        data.pickerArray[1] = day;
      }
      this.setData(data);
    },
